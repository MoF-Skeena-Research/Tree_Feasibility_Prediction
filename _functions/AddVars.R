addVars <- function(dat){
  dat$PPT_MJ <- dat$PPT_05 + dat$PPT_06  # MaY/June precip
  dat$PPT_JAS <- dat$PPT_07 + dat$PPT_08 + dat$PPT_09  # July/Aug/Sept precip
  ...
  dat$CMDMax <- dat$CMD_07
  dat$CMD.total <- dat$CMD.def + dat$CMD
  dat$CMD.grow <- dat$CMD_05 + dat$CMD_06 +dat$CMD_07 +dat$CMD_08 +dat$CMD_09
  dat$DD5.grow <- dat$DD5_05 + dat$DD5_06 + dat$DD5_07 + dat$DD5_08 + dat$DD5_09
  dat$CMDMax <- dat$CMD_07 # add in so not removed below
